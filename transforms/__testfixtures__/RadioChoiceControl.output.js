...
|}> {
  ...
  render() {
    ...
    const controlClassName = classNames(classes.control, classes.control);
    const Wrapper = classes.rowWrapper ? 'div' : React.Fragment;

    return <>
      {this.renderLabel()}
      <div className={classNames(classes.root, classes.root)}>
        {this.props.choices.map(
          ({ id, title, classValidate = false }, index) => {
            const baseLabelClassName = classNames(classes.label, classes.label, {
